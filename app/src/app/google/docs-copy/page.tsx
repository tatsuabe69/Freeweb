...
export default function DocsCopyPage() {
  ...
  const handleCopy = async () => {
    ...
    if (!fileId) {
      setError("正しいGoogleドキュメントのURLを入力してください");
      return;
    }
  ...
  return (
    <div className="mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-10 py-6 sm:py-8">
      ...
      <div className="flex items-center gap-3 mb-1">
        <Copy className="h-5 w-5 text-[#4285f4]" />
        <h1 className="text-xl font-semibold tracking-tight">Googleドキュメント複製</h1>
      </div>
      <p className="text-sm text-muted-foreground mb-8">
        テンプレートのドキュメントを複数チーム分まとめてコピー。研修・ワークショップの準備を効率化。
      </p>
      ...
      {results.length === 0 ? (
        <div className="space-y-6">
          ...
          {/* ドキュメントURL */}
          <div>
            <label className="text-sm font-medium mb-2 block">コピー元のGoogleドキュメントURL</label>
            <input
              type="url"
              value={docUrl}
              ...
              <div className="text-sm text-muted-foreground space-y-4 pt-2">
                ...
                <div>
                  ...
                  <div className="relative group">
                    ...
                    <pre id="gas-code-docs" className="bg-background border rounded-lg p-3 pt-10 text-xs overflow-x-auto whitespace-pre">{`function doGet(e) {
            ...
            {results.map((r, i) => (
              ...
              >
                <div className="w-8 h-8 rounded-lg bg-[#4285f4]/10 flex items-center justify-center shrink-0">
                  <span className="text-sm font-medium text-[#4285f4]">{i + 1}</span>
                </div>
